...
| `NEXT_PUBLIC_HISTORICAL_RPC_LOG_BLOCK_BATCH_SIZE`        | Historical `eth_getLogs` batch block range size, defaults to `10000` (clamped to `500..50000`) |
| `NEXT_PUBLIC_HISTORICAL_RPC_LOG_MAX_CONCURRENT_REQUESTS` | Max concurrent historical `eth_getLogs` batch requests, defaults to `3` (clamped to `1..5`)    |
